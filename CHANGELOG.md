...
- styles/builtin/ironic.shipstyle: `.doc-narrative > h1:first-child { display: none }` in both changelog and story — hides redundant title heading generated by LLM/goldmark
- styles/builtin/ironic.shipstyle: icons updated — feature:`+`, fix:`~`, breaking:`!`, chore/docs/test:`·`, story_peak:`↑` — distinct per type instead of all `→`
- Tests: 7 new (palette colors, donut centering, narrative h1 hidden, epic separators, ironic icon assertions)
