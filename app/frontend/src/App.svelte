      ...
      <div class="brand" style="--wails-draggable: no-drag">
        {#if theme.logo}
          <span class="brand-logo">
            {#if theme.logo.trim().startsWith('<svg')}
              {@html theme.logo}
            {:else}
    ...
    <div class="nav-section" style="--wails-draggable: no-drag">
      ...
      {#each viewScreens as screen}
        <button class="nav-item" class:active={activeScreen === screen.name} on:click={() => activeScreen = screen.name}>
          ...
          <span class="nav-text">{screen.name === 'Dashboard' ? $uiLabels.dashboard : screen.name === 'Generate' ? $uiLabels.generate : screen.name === 'Story' ? $uiLabels.story : screen.name === 'History' ? $uiLabels.history : $uiLabels.contributors}</span>
        </button>
      {/each}
